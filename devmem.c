...
void usage(void)
{
	printf("*argv[0] -g <REG_ADDRESS> -i|-o <VALUE>\n");
	printf("    -g <REG_ADDR>   GPIO physical address\n");
...
int main(int argc, char *argv[])
{
	...
	while((c = getopt(argc, argv, "g:io:h")) != -1) {
		...
		case 'g':
			...
			break;
		case 'i':
			direction=IN;    		//Read reg
			break;
		case 'o':
			direction=OUT; 			//Write reg
			value=atoi(optarg); 	//convert int
			break;
		case 'h':      				//if not -g -i or -o
			usage();
			return 0;
		default:
